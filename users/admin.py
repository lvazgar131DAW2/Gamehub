...
class UserGameInline(admin.TabularInline):
    """Inline para mostrar juegos del usuario"""
    model = Game
    extra = 0
...
class UserReviewInline(admin.TabularInline):
    """Inline para mostrar reseñas del usuario"""
    model = Review
    extra = 0
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    def username_colored(self, obj):
        """Muestra usuario con color según si es staff"""
        if obj.is_staff:
            color = '#FFE66D'
            prefix = '👑 '
        else:
    ...
    def games_count(self, obj):
        """Muestra número de juegos creados"""
    ...
    def last_login_display(self, obj):
        """Muestra último login formateado"""
        if obj.last_login:
